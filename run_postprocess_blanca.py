...
indir = '/scratch/alpine/ikmi3774/slurm/chains_parrot_UDS_n3.0_v1.2_ACS+WEBB_Kf444w_SUPER_spsbeta_massive20000/'
outdir = '/scratch/alpine/ikmi3774/slurm/postprocess_UDS_n3.0_v1.2_massive20000/'
log_dir = '/scratch/alpine/ikmi3774/slurm/postprocess_UDS_n3.0_v1.2_massive20000/logs/'
...
if process==2: ### broken files, need ids of the broken file
    # this is added to re-run several sources, which is broken in either spec or perc. typically ~a few % of the sources
    ids = ids
    print(len(ids))
    for j in range(0,len(ids)):
        print("rm "+outdir+"npz/id_"+str(int(ids[j]))+"_*_phisfh.h5")
        os.system("rm "+outdir+"npz/id_"+str(int(ids[j]))+"_*_phisfh.h5")
    np.savetxt(f'{outdir}/id_files/ids_postprocess_9999.txt', ids, fmt='%d')
